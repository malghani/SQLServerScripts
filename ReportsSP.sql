...
if NOT Exists(Select * from sys.objects where name='sal002_tier1_sales') OR ((select DB_ID())<=4)
    ...
    SET NOEXEC OFF;
GO
/****** Object:  StoredProcedure [dbo].[SSRS_SalesTier1_LOB_Revenue_By_Category]    Script Date: 10/21/2013 11:55:29 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SSRS_SalesTier1_LOB_Revenue_By_Category]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SSRS_SalesTier1_LOB_Revenue_By_Category]
GO
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROC SSRS_SalesTier1_LOB_Revenue_By_Prod 
...
SELECT 
 ...
 AND ISNULL(Super_Category,'No Category') = ISNULL(@Cat, ISNULL(Super_Category,'No Category'))
...
WITH a 
     ...
     AS (SELECT lob, 
                ...
                Sum(amount)                            AS Amount, 
                Isnull(lob, 'No LOB') 
                + Isnull(account_group, 'No Account_Group') + efg_rep 
...
(SELECT Isnull(lob, 'No LOB') AS
   LOB, efg_rep, Isnull(account_group, 'No Group') AS Account_Group, 
   dealer_name,  admtrntyp, super_category, cprod, 
   Count(DISTINCT admclicod + admacccod + admagrcod + admagrsfx) AS Count, 
   Sum(Convert(float, ISNULL(disbursement_amount,0))) AS Amount 
FROM   dbo.sal002_tier1_sales 
...
(SELECT lob, 
   ...
   Sum(( CASE a.admtrntyp 
    ...
    END ))AS Count, 
	Sum(amount) AS Amount,  
	Isnull(lob, 'No LOB') + Isnull(account_group, 'No Group') + efg_rep +
	dealer_name + super_category + cprod AS ID 
 FROM   a 
 WHERE ISNULL(LOB, 'No LOB')=ISNULL(@LOB, ISNULL(LOB, 'No LOB'))
	AND ISNULL(Account_Group, 'No Account_Group')=ISNULL(@AccountGroup, ISNULL(Account_Group, 'No Account_Group'))
...
(SELECT Isnull(lob, 'No LOB')AS LOB, efg_rep, 
 ...
 WHERE  Substring(CONVERT(VARCHAR, admpstdat), 5, 2) = Datepart(mm, Getdate()) 
  ...
  GROUP  BY lob, efg_rep, account_group, dealer_name, admtrntyp, super_category, 
	cprod, Isnull(lob, 'No LOB') + Isnull(account_group, 'No Group') + efg_rep 
                   + dealer_name + super_category + cprod
),
d AS 
(SELECT lob, efg_rep, account_group, dealer_name, super_category, cprod, 
...
/*
        Reports Consuming  :  
				...
				exec SSRS_SalesTier1_Gross_Monthly_Account_Performance_Detail @LOB='Agency Services', @EFG_Rep='Paul Roberts', @AccountGroup='No Account_Group'
				
						
				Gross Monthly Account Performance by Account
				exec SSRS_SalesTier1_Gross_Monthly_Account_Performance_Detail @LOB='Agency Services', @EFG_Rep='Paul Roberts', @AccountGroup='No Account_Group', @Account='Aristocrat Motors' 
